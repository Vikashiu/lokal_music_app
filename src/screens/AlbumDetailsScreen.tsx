...
// // API to get full album details + songs
// const ALBUM_DETAILS_API = (id: string) => `https://saavn.sumit.co/api/albums?id=${id}`;

// export default function AlbumDetailsScreen() {
//   const navigation = useNavigation();
//   const route = useRoute();
//   const { album } = route.params as { album: any }; // Data passed from tab
...
//     try {
//       const response = await fetch(ALBUM_DETAILS_API(album.id));
//       const json = await response.json();

//       if (json.success && json.data && json.data.songs) {
//          const mappedSongs: Song[] = json.data.songs.map((item: any) => {
...
//                 artist: item.artists?.primary?.[0]?.name || album.artist,
...
//           <Image source={{ uri: album.image }} style={styles.heroImage} />
//           <Text style={styles.title}>{album.name}</Text>
//           <Text style={styles.stats}>
//              1 Album  |  {songs.length} Songs  |  {album.year}
//           </Text>
...
//                         // Modal is automatic via SongListItem!
//                     />
//                 ))}
//             </View>
//         )}
...
//   heroImage: { width: 220, height: 220, borderRadius: 24, marginBottom: 16, shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 10, elevation: 10 },
//   title: { fontSize: 22, fontWeight: 'bold', color: '#000', marginBottom: 6, textAlign: 'center', paddingHorizontal: 20 },
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {/* Hero Section matches Starboy styling */}
        <View style={styles.heroContainer}>
          <Image source={{ uri: album.image }} style={styles.heroImage} />
          <Text style={[styles.title, { color: theme.text }]}>{album.name}</Text>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity style={[styles.playAllButton, { backgroundColor: isDarkMode ? 'rgba(255, 107, 0, 0.15)' : '#FFF0E0' }]}>
            ...
            <View style={{ paddingHorizontal: 20 }}>
                {songs.map((song, index) => (
                    <SongListItem 
                        key={index} 
                        song={song} 
                        onPlay={() => playSong(song, songs)} // Pass songs array to populate queue
                    />
                ))}
            </View>
        )}
...
const styles = StyleSheet.create({
  ...
  topIcons: { flexDirection: 'row' },
  heroContainer: { alignItems: 'center', marginBottom: 30 },
  heroImage: { width: 220, height: 220, borderRadius: 32, marginBottom: 16 }, // Large rounded corners
  title: { fontSize: 22, fontWeight: 'bold', marginBottom: 6, textAlign: 'center', paddingHorizontal: 20 },
